...
PWD=exasol
EXAPLUS=exaplus
BOOTSTRAP=`dirname $0`/ddl/bootstrap.sql
...
$EXAPLUS -c $DSN -u $USER -p $PWD -sql "export (select ddl from db_history.database_ddl) into local csv file '$BOOTSTRAP' DELIMIT=NEVER;"
